  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-slate-200/50 text-left text-xs text-slate-500">
        ...
        <tbody className="divide-y divide-slate-700/30">
          {newestFirst.map((entry) => (
            <tr key={entry.id} className="transition-colors hover:bg-slate-100/20">
              <td className="py-3 pr-4 whitespace-nowrap text-slate-600 font-medium">
              ...
              <td className="py-3 pr-4 text-slate-500 max-w-xs truncate" title={entry.notes}>
                {entry.notes || '—'}
              </td>
              <td className="py-3 text-right whitespace-nowrap">
                <button
                  onClick={() => onEdit(entry)}
                  className="mr-3 text-xs text-slate-600 transition hover:text-slate-400"
                >
                  Edit
                </button>
                <button
